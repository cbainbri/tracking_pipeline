...
class TrackPosition:
    ...
    # Shape metrics for optogenetics experiments
...
class TrackManager:
    ...
    def __init__(self, experiment_type: str = 'food'):
        ...
        self.has_shape_data = False  # For optogenetics shape metrics
        self.experiment_type = experiment_type
        # Stim tracking for optogenetics experiments
        self.stim_on_frame = None
        self.stim_off_frame = None
        self.has_stim_data = False
    ...
    def load_from_dataframe(self, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            shape_columns = [col for col in df.columns 
                           if col.startswith('worm_') and any(x in col for x in ['_major_axis', '_minor_axis', '_aspect_ratio', '_convexity', '_solidity'])]
            self.has_shape_data = len(shape_columns) > 0
            if self.has_shape_data:
                logger.info(f"Found shape metric data")
            ...
            for track_id in sorted(track_ids):
                ...
                # Shape metric columns
                major_axis_col = f'worm_{track_id}_major_axis'
                minor_axis_col = f'worm_{track_id}_minor_axis'
                aspect_ratio_col = f'worm_{track_id}_aspect_ratio'
                area_col = f'worm_{track_id}_area'
                perimeter_col = f'worm_{track_id}_perimeter'
                convexity_col = f'worm_{track_id}_convexity'
                solidity_col = f'worm_{track_id}_solidity'
                ...
                if x_col in df.columns and y_col in df.columns:
                    ...
                    for _, row in df.iterrows():
                        ...
                        if pd.notna(x) and pd.notna(y):
                            ...
                            # Load shape data if present
                            major_axis = float(row[major_axis_col]) if major_axis_col in df.columns and pd.notna(row[major_axis_col]) else None
                            minor_axis = float(row[minor_axis_col]) if minor_axis_col in df.columns and pd.notna(row[minor_axis_col]) else None
                            aspect_ratio = float(row[aspect_ratio_col]) if aspect_ratio_col in df.columns and pd.notna(row[aspect_ratio_col]) else None
                            area = float(row[area_col]) if area_col in df.columns and pd.notna(row[area_col]) else None
                            perimeter = float(row[perimeter_col]) if perimeter_col in df.columns and pd.notna(row[perimeter_col]) else None
                            convexity = float(row[convexity_col]) if convexity_col in df.columns and pd.notna(row[convexity_col]) else None
                            solidity = float(row[solidity_col]) if solidity_col in df.columns and pd.notna(row[solidity_col]) else None
                            
                            positions.append(TrackPosition(
                                float(x), float(y), frame, nose_x, nose_y,
                                major_axis, minor_axis, aspect_ratio, area, perimeter, convexity, solidity
                            ))
            ...
            # Check for stim data
            stim_columns = [col for col in df.columns if '_stim' in col and col.startswith('worm_')]
            if stim_columns:
                self.has_stim_data = True
                # Find stim ON frame (first frame where stim=1)
                # Find stim OFF frame (first frame after ON where stim=0)
                for _, row in df.iterrows():
                    frame = int(row['frame'])
                    stim_val = row[stim_columns[0]]
                    
                    if pd.notna(stim_val):
                        if stim_val == 1 and self.stim_on_frame is None:
                            self.stim_on_frame = frame
                        elif stim_val == 0 and self.stim_on_frame is not None and self.stim_off_frame is None:
                            self.stim_off_frame = frame
                
                if self.has_stim_data:
                    logger.info(f"Found stim data: ON={self.stim_on_frame}, OFF={self.stim_off_frame}")
    ...
    def export_to_dataframe(self) -> pd.DataFrame:
        ...
        for track_id in sorted(self.tracks.keys()):
            ...
            # Add shape data if present AND in optogenetics mode
            if self.has_shape_data and self.experiment_type == 'opto':
                major_axis_col = f'worm_{track_id}_major_axis'
                minor_axis_col = f'worm_{track_id}_minor_axis'
                aspect_ratio_col = f'worm_{track_id}_aspect_ratio'
                area_col = f'worm_{track_id}_area'
                ...
                data[major_axis_col] = [pos_by_frame[f].major_axis if f in pos_by_frame else np.nan for f in all_frames]
                data[minor_axis_col] = [pos_by_frame[f].minor_axis if f in pos_by_frame else np.nan for f in all_frames]
                data[aspect_ratio_col] = [pos_by_frame[f].aspect_ratio if f in pos_by_frame else np.nan for f in all_frames]
                data[area_col] = [pos_by_frame[f].area if f in pos_by_frame else np.nan for f in all_frames]
                data[perimeter_col] = [pos_by_frame[f].perimeter if f in pos_by_frame else np.nan for f in all_frames]
                data[convexity_col] = [pos_by_frame[f].convexity if f in pos_by_frame else np.nan for f in all_frames]
                data[solidity_col] = [pos_by_frame[f].solidity if f in pos_by_frame else np.nan for f in all_frames]
            
            # Add stim data if present
            if self.has_stim_data:
                stim_col = f'worm_{track_id}_stim'
                stim_values = []
                for f in all_frames:
                    stim_val = 0
                    # Check if frame is in stim period
                    if self.stim_on_frame is not None and f >= self.stim_on_frame:
                        stim_val = 1
                    if self.stim_off_frame is not None and f >= self.stim_off_frame:
                        stim_val = 0
                    stim_values.append(stim_val)
                data[stim_col] = stim_values
...
class OptimizedTrackEditor:
    ...
    def __init__(self):
        ...
        # For 1080p (1920x1080) or smaller, use smaller window
        if screen_height <= 1080:
            window_height = 850  # Slightly smaller to fit on 1080p screens
        else:
            window_height = 900
        
        self.root.geometry(f"1600x{window_height}")
        
        # Ask for experiment type first
        self.experiment_type = self.ask_experiment_type()
        
        # Create track manager with experiment type
        self.track_manager = TrackManager(experiment_type=self.experiment_type)
    ...
    def ask_experiment_type(self) -> str:
        """Ask user for experiment type"""
        dialog = tk.Toplevel(self.root)
        dialog.title("Experiment Type")
        ...
        tk.Label(dialog, text="Select experiment type:", font=('TkDefaultFont', 10, 'bold')).pack(pady=10)
        
        experiment_var = tk.StringVar(value="food")
        
        tk.Radiobutton(dialog, text="Food Encounter", variable=experiment_var, value="food", 
                      font=('TkDefaultFont', 10)).pack(anchor='w', padx=40, pady=5)
        tk.Radiobutton(dialog, text="Optogenetics", variable=experiment_var, value="opto",
                      font=('TkDefaultFont', 10)).pack(anchor='w', padx=40, pady=5)
    ...
    def setup_gui(self):
        ...
        # Left panel: Controls (scrollable for 1080p compatibility)
        left_outer = ttk.Frame(main_frame, width=300)
        left_outer.pack(side='left', fill='y', padx=5, pady=5)
        left_outer.pack_propagate(False)
        
        # Create canvas and scrollbar for left panel
        self.control_canvas = tk.Canvas(left_outer, width=280, highlightthickness=0)
        scrollbar = ttk.Scrollbar(left_outer, orient='vertical', command=self.control_canvas.yview)
        ...
        # Setup left panel controls (in scrollable frame)
        self.setup_controls(self.scrollable_frame)
    ...
    def setup_controls(self, parent):
        ...
        file_frame = ttk.LabelFrame(parent, text="File Loading", padding=5)
        ...
        playback_frame = ttk.LabelFrame(parent, text="Playback", padding=5)
        ...
        edit_frame = ttk.LabelFrame(parent, text="Track Editing", padding=5)
        ...
        # REDUCED height to 6 for compact display on 1080p screens
        ...
        # Stim marking buttons (only for optogenetics experiments)
        if self.experiment_type == "opto":
            ttk.Label(edit_frame, text="Stim Marking:", font=('TkDefaultFont', 9, 'bold')).pack(pady=(5,2))
            
            stim_btn_frame = ttk.Frame(edit_frame)
            stim_btn_frame.pack(fill='x', pady=1)
            
            ttk.Button(stim_btn_frame, text="Mark Stim ON", 
                      command=self.mark_stim_on).pack(side='left', padx=2, expand=True, fill='x')
            ttk.Button(stim_btn_frame, text="Mark Stim OFF", 
                      command=self.mark_stim_off).pack(side='left', padx=2, expand=True, fill='x')
            
            self.stim_label = ttk.Label(edit_frame, text="Stim: [ON: -, OFF: -]", 
                                       font=('TkDefaultFont', 8))
            self.stim_label.pack(pady=2)
            
            # Update stim label if data already loaded
            if self.track_manager.has_stim_data:
    ...
    def load_track_csv(self):
        ...
        try:
            ...
            if self.track_manager.load_from_dataframe(df):
                ...
                # Update stim label if stim data loaded
                if self.track_manager.has_stim_data and hasattr(self, 'stim_label'):
                    self.update_stim_label()
    ...
    def save_tracks(self):
        ...
        try:
            ...
            # Tracks are stored at original resolution - no scaling needed
    ...
    def mark_stim_on(self):
        """Mark current frame as stim onset"""
        if not self.image_cache.get_frame_count():
            messagebox.showwarning("No Images", "Load images first")
            return
        
        current_frame = self.current_frame
        self.track_manager.stim_on_frame = current_frame
        self.track_manager.has_stim_data = True
        self.update_stim_label()
        logger.info(f"Marked stim ON at frame: {current_frame}")
        self.status_label.config(text=f"Stim ON marked at frame {current_frame}")
    
    def mark_stim_off(self):
        """Mark current frame as stim offset"""
        if not self.image_cache.get_frame_count():
            messagebox.showwarning("No Images", "Load images first")
            return
        
        current_frame = self.current_frame
        self.track_manager.stim_off_frame = current_frame
        self.track_manager.has_stim_data = True
        self.update_stim_label()
        logger.info(f"Marked stim OFF at frame: {current_frame}")
        self.status_label.config(text=f"Stim OFF marked at frame {current_frame}")
    
    def update_stim_label(self):
        """Update stim label display"""
        if hasattr(self, 'stim_label'):
            on_text = f"ON: {self.track_manager.stim_on_frame}" if self.track_manager.stim_on_frame is not None else "ON: -"
            off_text = f"OFF: {self.track_manager.stim_off_frame}" if self.track_manager.stim_off_frame is not None else "OFF: -"
            self.stim_label.config(text=f"Stim: [{on_text}, {off_text}]")
