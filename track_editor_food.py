...
"""
ULTRA-OPTIMIZED TRACK EDITOR - Production Ready Version
...
class TrackManager:
            ...
            for track_id in sorted(track_ids):
                ...
                if x_col in df.columns and y_col in df.columns:
                    ...
                    if np.any(valid_mask):
                        positions = [
                            TrackPosition(float(x), float(y), int(f))
                            for x, y, f in zip(x_vals[valid_mask], y_vals[valid_mask], frames[valid_mask])
                        ]
    ...
    def get_tracks_at_frame(self, frame_idx: int, trail_length: int = Config.DEFAULT_TRAIL_LENGTH) -> Tuple[Dict[int, List[TrackPosition]], Dict[int, List[TrackPosition]]]:
        """Get active and inactive tracks at frame"""
        active_tracks = {}
        inactive_tracks = {}
...
class UltraOptimizedTrackEditor:
    ...
    def create_editing_controls(self, parent):
        ...
        editing_frame = ttk.LabelFrame(parent, text="🛠️ TRACK EDITING", padding=5)
    ...
    def load_csv_from_path(self, file_path: str) -> bool:
        ...
        try:
            ...
            if self.track_manager.load_from_dataframe(df):
                ...
                self.status_label.config(text=f"Tracks: {len(self.track_manager.tracks)} loaded")
                return True
            else:
    ...
    def update_frame_display(self, force_update=False, use_blitting=True):
        ...
        try:
            ...
            # Check if we jumped to a significantly different frame (coordinate system might have changed)
            ...
            can_use_blitting = (use_blitting and
                               ...
                               not frame_jump)  # Don't use blitting after big jumps
    ...
    def _blitted_track_update(self):
        ...
        active_tracks, inactive_tracks = self.track_manager.get_tracks_at_frame(
            self.current_frame, self.trail_length)
        ...
        for track_id, positions in active_tracks.items():
            ...
            # For trails: only use positions where we have proper coordinate data
            valid_display_positions = []
            
            if self.image_cache:
                # Only include positions that have valid coordinate transformations
                current_frame_ratio = self.image_cache.downsample_ratios.get(self.current_frame, 1.0)
                
                for pos in positions:
                    # Check if this position's frame has a known coordinate system
                    if pos.frame in self.image_cache.downsample_ratios:
                        # Use the position's own frame coordinate system
                        x, y = self.image_cache.get_display_coordinates(pos.x, pos.y, pos.frame)
                        valid_display_positions.append((x, y))
                    elif abs(pos.frame - self.current_frame) <= 2:
                        # For nearby frames without coordinate data, use current frame's system
                        x, y = pos.x * current_frame_ratio, pos.y * current_frame_ratio
                        valid_display_positions.append((x, y))
                    # Skip positions that are far from current frame and have no coordinate data
            else:
                # No image cache - use original coordinates
                valid_display_positions = [(pos.x, pos.y) for pos in positions]
            ...
            # Trails - only if we have multiple valid positions
            if self.show_trails_var.get() and len(valid_display_positions) > 1:
                xs, ys = zip(*valid_display_positions)
    ...
    def _draw_tracks_on_axis(self):
        ...
        for track_id, positions in active_tracks.items():
            ...
            # For trails: only use positions where we have proper coordinate data
            # This prevents mixing coordinate systems from different frame segments
            valid_display_positions = []
            
            if self.image_cache:
                # Only include positions that have valid coordinate transformations
                # or use a consistent fallback approach
                current_frame_ratio = self.image_cache.downsample_ratios.get(self.current_frame, 1.0)
            ...
            # Current position - use the last valid position
    ...
    def schedule_next_frame(self):
        ...
        # Force full update during playback to ensure images load
    ...
    def goto_frame(self, frame_idx):
        ...
        if new_frame != self.current_frame:
            # Check if this is a big jump that might cause coordinate issues
            ...
            # For big jumps, force a full redraw to prevent coordinate artifacts
    ...
    def on_canvas_click(self, event):
        ...
        active_tracks, inactive_tracks = self.track_manager.get_tracks_at_frame(self.current_frame)
